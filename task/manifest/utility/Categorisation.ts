...
type PlugCategorisationExpression =
	...
	| '*Deprecated'
	| `${DeepsightPlugCategoryName}/*Deprecated`
...
namespace Categorisation {
	...
	export const IsDeprecated = matcher('*Deprecated')
	...
	export function matcher (...expressions: (PlugCategorisationExpression | `!${PlugCategorisationExpression}`)[]) {
		...
		return function (categorised?: DeepsightPlugCategorisation | DeepsightPlugFullName): boolean {
			const categorisation = typeof categorised === 'string' ? categorised : categorised?.fullName
			if (!categorisation)
				return false
